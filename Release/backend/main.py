...
@app.get("/")
async def read_root():
    """프론트엔드 index.html 서빙 (프로덕션 모드)"""
    frontend_build_path = Path(__file__).parent.parent / "frontend" / "build"
    index_file = frontend_build_path / "index.html"
    if index_file.exists():
        return FileResponse(index_file)
    return {"Hello": "World", "message": "프론트엔드 빌드 파일이 없습니다. npm run build를 실행하세요."}

# SPA fallback - API가 아닌 모든 경로를 index.html로 라우팅
@app.get("/{full_path:path}")
async def serve_spa(full_path: str):
    """SPA fallback - 모든 비-API 경로를 프론트엔드로 라우팅"""
    # API 및 정적 파일 경로는 제외
    if full_path.startswith(("api/", "static/", "assets/", "token", "users")):
    ...
    if index_file.exists():
        return FileResponse(index_file)
    raise HTTPException(status_code=404, detail="프론트엔드 빌드 파일이 없습니다.")
